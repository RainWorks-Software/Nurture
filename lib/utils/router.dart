...
import 'package:openfoodfacts/openfoodfacts.dart' show Product;
...
final routerAndroid = GoRouter(
  routes: [
    ...
    GoRoute(
      path: "/no-camera-access",
      name: "no_camera_access",
      builder: (context, state) =>
          Android.NoPermissionPage(requiredPermission: Permission.camera),
    ),
    GoRoute(
      path: "/me",
      name: "me",
      builder: (context, state) => const Android.MePage(),
    ),
    GoRoute(
      path: "/ai",
      name: "ai",
      builder: (context, state) {
        Product productInfo = state.extra as Product;

        return Android.AiChatPrompt(product: productInfo);
      },
    ),
  ],
);
